...
class IK(object):
	...
	def compute(self, angles: list, target_pos: list, target_rot: list) -> np.ndarray:
		...
		trans_mat_current = self.fwd_kinematic(angles)
  
		# error computation
		error = np.linalg.norm(trans_mat_current - trans_mat_target)
  
		while error > 0.001:
			jacob_mat = self.get_jacob_mat(q[iter,0],q[iter,1],q[iter,2],q[iter,3],q[iter,4],q[iter,5])
			...
			if(np.any(q[-1,:] > pi) or np.any(q[-1,:] < -pi)): 
	...
	def run(self) -> np.ndarray:
		res = self.compute(self.angles, self.pos, self.rot)
		print("\nJoint angles:")
		print(res.round(2))

#%% main
np.set_printoptions(suppress=True)
init_joint_angles = [0, 0, 0, 0, 0, 0] # Degree
tcp_pos = [-120, -376, 200] # mm
tcp_rot = [0, 3.142, 0] # axis angle
